...
for f in dd_files:
    if os.path.exists(f):
        df = pd.read_csv(f)
        dd_dfs.append(df)
        print(f"   ✓ Loaded {f}: {len(df)} simulations")
    else:
        print(f"   ✗ Missing {f}")
...
if dd_dfs:
    ...
    print(f"\n   ✓ Merged {len(dd_merged)} simulations")
    ...
    print(f"\n   Best simulation: {best_sim} (L2={best_l2:.2f} µm)")
else:
    print("   ✗ No dd_study results found")
...
if os.path.exists(gabs_file):
    ...
    print(f"   ✓ Loaded {gabs_file}: {len(gabs_df)} simulations")
    print(f"\n   Summary Statistics:")
